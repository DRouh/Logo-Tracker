...
class BaseTracker:
  ...
  def run(self):           
    ...
    for i in range(len(self.Labels)):
        ...
        self.SavedImg.append(self.RefImagesBW[i])
    ...
    while True:
      ...
      for j in range(len(self.Labels)):
          found, boxes = self.detectLogo(j,self.Labels[j], features[j], self.RefImagesBW[j], orig_img, gray_img, img_Sift, frameKp, frameDescs)
  ...
  def detectLogo(self,num,  label, feature,ref_img, orig_img, gray_img, img_Sift, frameKp, frameDescs):
      ...
      while(cont):
          ...
          if inl == None or matches == None:
              if len(self.SavedKpDescs) > 0:                 
                  print "querying from saved"                 
              break
          ...
          if matches >= 100 and (matches >= 200 or score > 0.48 or inl > 100):             
              ...
              self.Bestscore = score if score > self.Bestscore else score
              self.SavedImg[num] = gray_img[box[1][1]:box[0][1], box[1][0]:box[2][0]]
              cv2.imshow("cropped", self.SavedImg[num])
              
              fKp, fDes = self.DeleteKeypoints(fKp, fDes, box[1][0], box[1][1], box[3][0], box[3][1])              
          elif len(self.SavedImg) > 0:
              print "Querying saved image"
              rKps,rDs = self.AsiftMatcher.affine_detect(self.Detector, self.SavedImg[num], mask=None, pool=self.Pool)
              inl, matches, matchedKp = self.AsiftMatcher.asift_match(ref_img, gray_img, rKps,rDs, fKp, fDes)
              if matches >= 100 and (matches >= 200 or score > 0.48 or inl > 100):         
                  print "!matches found during querying"
                  #cont = True
                  found += 1
                  box = self.MinRectByMatchedKeypoints(matchedKp)
                  scores.append(score)
